...
#    Updated: 2021/02/11 07:05:23 by lfourmau         ###   ########lyon.fr    #
#                                                                              #
# **************************************************************************** #
FROM  debian:buster
COPY /srcs .
...
RUN apt-get install -y nginx \
	wget \
	php-cli php-mysql php-curl php-gd php-intl php-json php-mbstring php-zip php-pear php-gettext php-xml php-gd php-cgi php-fpm \
	mariadb-server mariadb-client \
	openssl

RUN service mysql start
#nginx
RUN rm /etc/nginx/sites-enabled/default && mv nginx.conf /etc/nginx/sites-enabled/
#phpmyadmin
...
&& cd /var/www/html/phpmyadmin/ && chmod 660 config.inc.php && rm config.sample.inc.php
...
&& rm /var/www/html/latest-fr_FR.tar.gz && rm /var/www//html/wordpress/wp-config-sample.php
